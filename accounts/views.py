        ...
        if form.is_valid():
            ...
            if password1 == password2 and check_password(password=old_password,
                                                         ...
                                                         setter=make_password(password=old_password, salt=None,
                                                                              hasher='default')):
                user = request.user
                user.password = make_password(password=password1, salt=None, hasher='default')
                user.save()
                update_session_auth_hash(request, user)
                messages.success(request, "Hasło zostało zmienione.")
            else:
                messages.error(request, "Obecne hasło jest nieprawidłowe lub nowe hasła się nie zgadzają.")
        else:
            messages.error(request, "Formularz został wypełniony nieprawidłowo.")
    return HttpResponseRedirect(reverse('settings'))